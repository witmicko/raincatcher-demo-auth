...
module.exports = function(mediator, store) {
  ...
  var topicVerify = 'wfm:user:verifyPassword';
  console.log('Subscribing to mediator topic:', topicVerify);
  mediator.subscribe(topicVerify, function(id, pwd) {
    store.verifyPassword(id, pwd, function(err, passwordCorrect) {
      if (err) {
        return mediator.publish('error:' + topicVerify + ':' + id, 'User not found');
      }
      mediator.publish('done:' + topicVerify + ':' + id, passwordCorrect);
    });
  });
  ...
  mediator.subscribe(topicDelete, function(user) {
    const id = user.id;
    store.delete(user, function(err, user) {
      if (err) {
        return mediator.publish('error:' + topicDelete + ':' + id, 'No such user');
      }
      mediator.publish('done:' + topicDelete + ':' + id, user);
    });
  });
