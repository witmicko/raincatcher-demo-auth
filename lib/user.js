...
module.exports = function(mediator, store) {
  ...
  mediator.subscribe(topicAuth, function(username, pwd) {
    store.verifyPassword(username, pwd, function(err, passwordCorrect) {
      if (err) {
        return mediator.publish('error:' + topicAuth + ':' + username, 'User not found');
      }
      mediator.publish('done:' + topicAuth + ':' + username, passwordCorrect);
    });
  });

  var topicPasswordEdit = 'wfm:user:password';
  console.log('Subscribing to mediator topic:', topicPasswordEdit);
  mediator.subscribe(topicPasswordEdit, function(username, oldPwd, newPwd) {
    store.updatePassword(username, oldPwd, newPwd, function(err, user) {
      if (err) {
        return mediator.publish('error:' + topicPasswordEdit + ':' + username, 'User not found');
      }
      mediator.publish('done:' + topicPasswordEdit + ':' + username, user);
    });
  });
